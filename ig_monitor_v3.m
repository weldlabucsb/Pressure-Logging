function ig_monitor_v3
%Author: Jeremy Max and Cora
%The improvements to this code include adding an interlock to close the
%gate valve in the case of too high pressure. This is quite a lot of power
%for the program to have...
%need to be interfacing with the NI board to send the TTL signal
...
mytimer = timer('Period',        updatePeriod,...
    ...
    function updateValues(~,~)
        ...
        tt.String = out2;
        thisday = date;
        foldername = strcat('Z:\Strontium Pressure Logs\',datestr(thisday,'yyyy-mmm'));
        if ~exist(foldername, 'dir')
           mkdir(foldername)
        end
        filename = strcat('Z:\Strontium Pressure Logs\',datestr(thisday,'yyyy-mmm'),'\',datestr(thisday,'yyyy-mmm-dd'),'.csv');
